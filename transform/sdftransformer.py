	...
	for actor in actors.keys():
		...
		if port_count['readers'] > 0 :
			...
			for count in range(port_count['readers']):
				...
				new_ports['pRead']={'name':'pRead','type':'in','rate':'1'}
				dst_actor = actor
				...
				if count == 0:
					...
					if (port_count['writers'] > 0):
						...
						if (port_rate('in',count,actors[actor])==1 or port_rate('out',(port_count['writers']-1),actors[actor])==1):
							### no sync needed ###
							new_ports['pBackward']={'name':'pBackward','type':'in','rate':str(port_rate('out',(port_count['writers']-1),actors[actor]))}
							channel_name = back_loop_actor + '__to__' + new_actor
							channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':new_actor,'dstPort':'pBackward','initialTokens':str(port_rate('out',(port_count['writers']-1),actors[actor])*port_rate('in',count,actors[actor]))}
							new_channels[channel_name] = channel_val
							new_channel_sizes[channel_name]=maxsize

						else:
							### create sync thread ###
							sync_name = 'sync_from__'+back_loop_actor+"__to__"+new_actor
							sync_ports={}
							sync_rate=port_rate('out',(port_count['writers']-1),actors[actor])*port_rate('in',count,actors[actor])
							sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
							...
							sync_ports['pForward']={'name':'pForward','type':'out','rate':str(sync_rate)}
							new_ports['pBackward']={'name':'pBackward','type':'in','rate':str(port_rate('out',(port_count['writers']-1),actors[actor]))}
							channel_name = sync_name + '__to__' + new_actor
							channel_val = {'name':channel_name,'srcActor':sync_name,'srcPort':'pForward','dstActor':new_actor,'dstPort':'pBackward','initialTokens':str(sync_rate)}
							new_channels[channel_name] = channel_val
							new_channel_sizes[channel_name]=maxsize
					...
					### in case there are no writers ###
						...
						sync_ports={}
						sync_rate=port_rate('in',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward'}
						new_channels[channel_name] = channel_val
						...
						sync_name = 'main_actor_from__'+back_loop_actor+"__to__"+new_actor
						sync_ports={}
						sync_rate=port_rate('in',(port_count['readers']-1),actors[actor])*port_rate('in',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward'}
						new_channels[channel_name] = channel_val
						...
						sync_ports['pForward']={'name':'pForward','type':'out','rate':str(sync_rate)}
						new_ports['pBackward']={'name':'pBackward','type':'in','rate':str(port_rate('in',(port_count['readers']-1),actors[actor]))}
						channel_name = sync_name + '__to__' + new_actor
						channel_val = {'name':channel_name,'srcActor':sync_name,'srcPort':'pForward','dstActor':new_actor,'dstPort':'pBackward','initialTokens':str(sync_rate)}
						new_channels[channel_name] = channel_val
						new_channel_sizes[channel_name]=maxsize
					...
					### check if sync is needed ###
					if (port_rate('in',count,actors[actor])==1 or port_rate('in',(count-1),actors[actor])==1):
						### no sync needed ###
						new_ports['pBackward']={'name':'pBackward','type':'in','rate':str(port_rate('in',(count-1),actors[actor]))}
						channel_name = back_loop_actor + '__to__' + new_actor
						...
						sync_name = 'sync_from__'+back_loop_actor+"__to__"+new_actor
						sync_ports={}
						sync_rate=port_rate('in',(count-1),actors[actor])*port_rate('in',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward'}
						new_channels[channel_name] = channel_val
				...
				if count == port_count['readers']-1:
					### check for front actor in loop ###
					if (port_count['writers'] > 0):
						### check if sync exists ###
						new_ports['pForward']={'name':'pForward','type':'out','rate':str(port_rate('out',0,actors[actor]))}
						new_actors[new_actor] = new_ports
					...
					### in case there are no writers ###
						...
						### should be connected to the first reader ###
						new_ports['pForward']={'name':'pForward','type':'out','rate':str(port_rate('in',0,actors[actor]))}
						new_actors[new_actor] = new_ports
				else:
					### should be connected to the next reader ###
					new_ports['pForward']={'name':'pForward','type':'out','rate':str(port_rate('in',count+1,actors[actor]))}
					new_actors[new_actor] = new_ports
		...
		if port_count['writers'] > 0 :
			...
			for count in range(port_count['writers']):
				...
				### the write port ###
				new_ports['pWrite']={'name':'pWrite','type':'out','rate':'1'}
				...
				if count == 0:
					...
					if (port_count['readers'] > 0):
						...
						sync_name = 'main_actor_from__'+back_loop_actor+"__to__"+new_actor
						sync_ports={}
						sync_rate=port_rate('in',(port_count['readers']-1),actors[actor])*port_rate('out',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward'}
						new_channels[channel_name] = channel_val
					...
					### in case there are no readers ###
						...
						sync_name = 'main_actor_from__'+back_loop_actor+"__to__"+new_actor
						sync_ports={}
						sync_rate=port_rate('out',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward','initialTokens':str(sync_rate)}
						new_channels[channel_name] = channel_val
						new_channel_sizes[channel_name]=maxsize
						...
						sync_name = 'main_actor_from__'+back_loop_actor+"__to__"+new_actor
						sync_ports={}
						sync_rate=port_rate('out',(port_count['writers']-1),actors[actor])*port_rate('out',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward','initialTokens':str(sync_rate)}
						new_channels[channel_name] = channel_val
					...
					### check if sync is needed ###
					if (port_rate('out',count,actors[actor])==1 or port_rate('out',(count-1),actors[actor])==1):
						### no sync needed ###
						new_ports['pBackward']={'name':'pBackward','type':'in','rate':str(port_rate('out',(count-1),actors[actor]))}
						channel_name = back_loop_actor + '__to__' + new_actor
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':new_actor,'dstPort':'pBackward'}
						new_channels[channel_name] = channel_val
						new_channel_sizes[channel_name]=maxsize
						...
						sync_name = 'sync_from__'+back_loop_actor+"__to__"+new_actor
						sync_ports={}
						sync_rate=port_rate('out',(count-1),actors[actor])*port_rate('out',count,actors[actor])
						sync_ports['pBackward']={'name':'pBackward','type':'in','rate':str(sync_rate)}
						channel_name = back_loop_actor + '__to__' + sync_name
						channel_val = {'name':channel_name,'srcActor':back_loop_actor,'srcPort':'pForward','dstActor':sync_name,'dstPort':'pBackward'}
						new_channels[channel_name] = channel_val
				...
				if count == port_count['writers']-1:
					...
					### in case there are no readers ###
						...
						### should be connected to the first writer ###
						new_ports['pForward']={'name':'pForward','type':'out','rate':str(port_rate('out',0,actors[actor]))}
						new_actors[new_actor] = new_ports
				else:
					### should be connected to the next writer ###
					new_ports['pForward']={'name':'pForward','type':'out','rate':str(port_rate('out',count+1,actors[actor]))}
					new_actors[new_actor] = new_ports
