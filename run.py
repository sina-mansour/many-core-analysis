...
def sdf_child(file_name, sdf_analysis, output_path, in_path, print_lock):
    ...
    args = [sdf_analysis, '--graph', connect_path(in_path, file_name), '--algo',
            'buffersize', '--output', (connect_path(output_path, file_name)[:-3] + 'res')]
    ...
    try:
        ...
        parse_buffer.parse_buffer(connect_path(output_path,
                                (file_name[:-3] + 'res')),
                                connect_path(output_path,
                                (file_name[:-3] + 'buf')))
        parse_buffer.parse_buffer_to_csv(connect_path(output_path,
                                        (file_name[:-3] + 'res')),
                                        connect_path(output_path,
                                        'results.csv'))
    finally:
...
def run_all(inputs_path, output_path, temp_path, random_times, read_delay, write_delay):
    ...
    sdf_analysis = file_path + \
        '/sdf3/latest_version/build/release/Linux/bin/sdf3analysis-sdf'
    ...
    input_xmls = [x for x in (run_cmd('ls "' + inputs_path + '"')
                              [0].strip().split('\n')) if (re.match('.*\.xml$', x) != None)]

    ### run graph sorting algorithms (c code) ###
    args = [actor_analysis, temp_path, str(
    ...
    for xml in input_xmls:
        sdf_childs[xml] = Process(target=sdf_child, args=(
            xml, sdf_analysis, output_path, inputs_path, print_lock,))
...
def main():
    ...
    if (len(sys.argv) < 2):
        print ("Error: not enough arguments, run '" +
               sys.argv[0] + " --help' for instructions")
        sys.exit(0)
            ...
            if (command == '--help'):
                ...
                if (len(sys.argv) > i + 1):
                    try:
                        ...
                        print (
                            "Error: invalid input for '--random_count <integer>'")
                else:
    ...
    run_all(inputs_path, output_path, temp_path,
            random_times, read_delay, write_delay)
